...
#item(Product_id ) with their respective sales
...
#Most purchased item and how many times it was purchased by each customer
select userid, count(product_id) cnt  from sales where product_id =
(select top 1 product_id from sales group by product_id order by count(product_id) desc)
group by userid;

5.Which item wsas most popular for each customer?

#users and their number of orders of each product
...
#sales of user who have gold user signup
...
9. If buying each product generates points for eg. 5rs-2 zomato point and each product has different purchasing points for eg. for p1 5rs- 1 zomato point , for p2 10rs-5 zomato point and p3 5rs- 1 zomato point

Calculate total_points_earned buy each customer and for which product most points have been given till now.

#total_points_earned buy each customer
...
#product with most points earned till date
...
Who earned more Customer 1 or 3 and what was their points earning in their first year?
1 zomato point = 2rs
0.5 zomato point = 1rs

# one year Report of customers as a gold user
select a.userid, a.created_date, a.product_id, b.gold_signup_date from sales a inner join
...
#analysis of points earned by each user in 1 year who has gold membership
...
11.rnk all the transation of the customers.

select *, rank() over(partition by userid order by created_date ) rnk from sales;

12. rank all the transactions for each member whenever they are a zomato gold member for every non gold member transactioon mark as na.

#use case expression to differentiate non gold member from  gold member by providing them the value 0
...
#Use cast function to change data type(int to varchar)
